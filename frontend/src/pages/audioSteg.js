/**
 * StegX Audio Steganography Page — Rewritten from Scratch
 */
import { staggerIn } from '../three/animations.js';

export function renderAudioSteg(container) {
  const algos = [
    { n: 'LSB',              d: 'Sample-level bit replacement in PCM audio. Highest capacity but least secure.', c: 'var(--primary)' },
    { n: 'Phase Coding',     d: 'Modifies phase angles of frequency components. High audio fidelity.',          c: 'var(--secondary)' },
    { n: 'Echo Hiding',      d: 'Introduces controlled echoes to encode bits. Very subtle and natural.',         c: 'var(--accent)' },
    { n: 'Spread Spectrum',  d: 'Spreads data across frequency spectrum. Highest security but lower capacity.', c: 'var(--warning)' },
  ];
    ...
    <div class="cards-grid stagger-item" style="margin-bottom:32px;">
      ${algos
        ...
        <div class="float-card">
          <div style="font-size:28px; margin-bottom:12px; color:${a.c};">♪</div>
          <h3 style="font-family:var(--font-display); font-size:16px; margin-bottom:8px;">${a.n}</h3>
          <p class="text-sm text-muted">${a.d}</p>
          <a href="#/hide" class="btn btn-ghost btn-sm mt-16">Use ${a.n} →</a>
        </div>`
        )
        .join('')}
    </div>
