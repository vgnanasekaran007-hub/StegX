...
export function renderAIRecommend(container) {
  coverData = null; secretData = null;
  container.innerHTML = `
    <div class="page-header"><h1 class="page-title">🤖 AI Recommendation Engine</h1><p class="page-subtitle">Get intelligent algorithm recommendations based on your files and requirements</p></div>
    <div style="display:grid; grid-template-columns:1fr 1fr; gap:24px; margin-bottom:24px;">
      <div class="glass-panel stagger-item"><h3 class="section-title">📁 Cover File</h3>${createUploadZone('ai-cover', { label: 'Cover file', icon: '🖼️', accept: 'image/*,audio/*,video/*' })}</div>
      <div class="glass-panel stagger-item"><h3 class="section-title">🔒 Secret File (Optional)</h3>${createUploadZone('ai-secret', { label: 'Secret file (optional)', icon: '📄' })}</div>
    </div>
        ...
        <div style="display:grid; grid-template-columns:1fr 1fr; gap:24px;" class="stagger-item">
          ...
          <div class="glass-panel">
            ...
            <ul style="list-style:none; display:flex; flex-direction:column; gap:8px;">
              ${(data.recommendations || []).map(r => `<li style="font-size:13px; color:var(--text-secondary); padding:8px 12px; background:var(--bg-glass); border-radius:var(--radius-sm); border-left:3px solid var(--primary);">💡 ${r}</li>`).join('')}
            </ul>
          </div>
        </div>
        <div class="glass-panel mt-24 stagger-item">
          <h3 class="section-title">📈 Algorithm Capacity Comparison</h3>
          <table class="data-table"><thead><tr><th>Algorithm</th><th>Capacity</th><th>Fits?</th><th>Utilization</th></tr></thead><tbody>
            ${Object.entries(data.capacity_prediction || {}).map(([algo, info]) => `
              <tr><td style="font-weight:600; color:${algo === data.best_algorithm ? 'var(--accent)' : 'var(--text-primary)'};">${algo.toUpperCase()} ${algo === data.best_algorithm ? '⭐' : ''}</td>
              <td>${info.capacity}</td><td>${info.fits ? '<span class="tag tag-accent">✓</span>' : '<span class="tag tag-danger">✕</span>'}</td><td>${info.utilization}</td></tr>
            `).join('')}
          </tbody></table>
        </div>
        <div class="glass-panel mt-24 stagger-item">
